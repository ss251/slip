...
- **Contract** — one Compact smart contract (`contracts/slip.compact`): commit–reveal, with the round and the member id bound into every commitment so a seal cannot be replayed into another round or by another member. A compiling contract is the heartbeat of this repo; never merge with a broken compile.
